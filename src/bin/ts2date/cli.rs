use clap::{Parser, ValueHint};
use dfir_toolkit::common::HasVerboseFlag;
use log::LevelFilter;
use clio::{Input,Output};


#[derive(Parser, Debug)]
#[clap(name=env!("CARGO_BIN_NAME"), author, version, about, long_about = None)]
pub (crate) struct Cli {
    /// Name of the file to read, default read from stdin
    #[clap(value_parser, value_hint=ValueHint::FilePath, default_value="-", help="if no file inputed, read from stdin", display_order(100))]
    pub(crate) input_file: Input,

    #[clap(flatten)]
    pub (crate) verbose: clap_verbosity_flag::Verbosity,

    /// name of the file to which the output will be written, default write to stdout
    #[clap(default_value="-", help="if no target file given, write to stdout", value_hint=ValueHint::FilePath, value_parser)]
    pub(crate) output_file: Output,
}

impl HasVerboseFlag for Cli {
    fn log_level_filter(&self) -> LevelFilter {
        self.verbose.log_level_filter()
    }
}